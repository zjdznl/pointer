...
public class SmsApi {
    ...
    //配置您申请的KEY
    public static final String APPKEY = "08de0e253a393f73bbd0a9982ac75e50";
    ...
    //2.发送短信
    public static BaseJson sendMessage(String phone) {
        String url = "http://v.juhe.cn/sms/send";//请求接口地址
        Map params = new HashMap();//请求参数
        params.put("mobile", phone);//接收短信的手机号码
        params.put("tpl_id", "140654");//短信模板ID，请参考个人中心短信模板设置
        String code = SmsApi.generateCode(6);
        ...
        try {
            String jsonResult = net(url, params, "GET");
            ...
            return new BaseJson(errorCode, reason, code);
        } catch (Exception e) {
            e.printStackTrace();
        }

        return BaseJson.unknownError();
    }

    public static String generateCode(int length) {
        String validateCode = (int) (Math.random() * Math.pow(10, length)) + "";
        if (validateCode.length() < length)
            for (int i = 0; i < length - validateCode.length(); i++)
    ...
    public static void main(String[] args) {
        BaseJson result = SmsApi.sendMessage("15201703512");
